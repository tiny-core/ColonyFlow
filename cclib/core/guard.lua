...
function M.range(v, min, max, name)
  ...
  if v < min or v > max then
    error(("[Guard] '%s' deve estar entre %s e %s, recebeu %s")
      :format(name or "?", min, max, v), 2)
  end
  return v
end
...
function M.oneOf(v, list, name)
  ...
  error(("[Guard] '%s' deve ser um de [%s], recebeu '%s'")
    :format(name or "?", table.concat(opts, ", "), tostring(v)), 2)
end
...
function M.props(props, schema)
  ...
  for key, validator in pairs(schema) do
    ...
    if not ok then
      error(("[M.props] " .. err):gsub("%[Guard%] ", ""), 2)
    end
  end
  return props
end
